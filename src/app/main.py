...
def render_vulnerable_ports(db):
    ...
    if not df_vuln.empty:
        df_vuln['dstport'] = pd.to_numeric(df_vuln['dstport'], errors='coerce').fillna(0).astype(int)
        
        port_mapping = {
            21: 'FTP (21)', 22: 'SSH (22)', 23: 'Telnet (23)', 
            80: 'HTTP (80)', 110: 'POP3 (110)', 445: 'SMB (445)', 
            3306: 'MySQL (3306)', 3389: 'RDP (3389)', 8080: 'HTTP-alt (8080)'
        }
        ...
        if not df_vuln.empty:
            ...
            fig = px.bar(
                df_vuln, 
                x='port_name', y='count', color='action', 
...
def render_vue1_descriptive_analysis(db) -> None:
    ...
    with c1:
        ...
        selected_range = st.selectbox("Plage de ports (RFC 6056) :", options=list(port_ranges.keys()))
    ...
    with c2:
        ...
        if rule_filter_enabled:
            selected_rule = st.number_input("ID de la règle (policyid) :", min_value=1, value=34, step=1)
    ...
    with col_bar:
        ...
        fig_bar = px.bar(
            df_proto, x='proto', y='count', color='action', barmode='group',
            title="Volume par Protocole", color_discrete_map=color_map,
            category_orders={'action': ['Permit', 'Deny']}
        )
        st.plotly_chart(fig_bar, use_container_width=True)

    with col_time:
        df_time = df_vue1.groupby(['time_window', 'action'])['count'].sum().reset_index()
        fig_time = px.line(
            df_time, x='time_window', y='count', color='action', markers=True,
            title="Répartition Temporelle (Activité par heure)", color_discrete_map=color_map,
            category_orders={'action': ['Permit', 'Deny']}
        )
...
def render_statistics_section(db):
    ...
    if not df_external.empty:
        st.warning(f"{len(df_external)} événements détectés depuis des sources externes.")
...
def render_port_scan_analysis(db) -> None:
    ...
    if not df_ports.empty:
        ...
        if not df_suspects.empty:
            ...
            st.info("Aucun comportement de balayage multi-ports détecté pour le moment.")
    else:
